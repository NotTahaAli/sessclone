...
- Second review: the confirm queues the keys it replaced (the old tail, dropped chunks) into `storage_orphans` in its transaction instead of deleting them after commit, so the sweep's guards protect a key a row names again. Each pending row carries the pass that signed it: the presign draws a `pass` (or keeps one the request echoes) and answers it, the Collector echoes it on its later presigns and its confirm, and the confirm and its refusals take only their own pass's rows. A recorded confirm also queues its pass's unused keys (the first presign's tail when the pass went on to seal). A key queued from the ledger waits in `storage_orphans` until its `not_before`, its URL's expiry. A whole-file confirm smaller than the row's `sealed_bytes` is `stale_chunks`, not a rollback.
