...
- Each chunk's `sha256` rides along in the list too, and `readChunk` checks it after gunzipping, so the viewer and ticket 133's download share one checksum. A renewed link that names a different tail offset, or no longer holds the chunk, fails with "Reload" instead of reading at the wrong offsets.
