...
`admin` and `/join/[token]` keep an empty shell on purpose and say so in the file: `admin`'s gate is the secret (a frame saying "sessclone · Platform" ahead of a 404 confirms the area exists), and the join page's whole content *is* the session read. Both keep `instant = false`, which per Next's own docs only silences instant-navigation validation — the empty shell comes from the structure, and the comments say that rather than crediting the flag.
