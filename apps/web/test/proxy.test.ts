...
// tested there case by case; these prove the Proxy applies them. Every
// Location is absolute: Vercel's Proxy runtime throws on a relative one, which
// took `/` down on sessclone.com on 2026-09-25 while these tests passed.
...
test('behind a proxy, the Referer is compared with NEXT_PUBLIC_APP_URL', async () => {
  ...
  // And the redirect names the configured origin, so the browser stays on
  // its own host rather than being sent to the server's `localhost`.
  expect(
