...
In the Supabase project, under Authentication, URL Configuration, add
`<NEXT_PUBLIC_APP_URL>/auth/callback` to **Redirect URLs** and set **Site URL**
to `NEXT_PUBLIC_APP_URL`. A sign-in whose callback is not on that list is sent
to the Site URL instead; the app forwards a `/?code=…` it receives there on to
`/auth/callback`, but only the list entry makes the redirect Supabase's own.
